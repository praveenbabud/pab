...
function addbook($link, $isbn13, $title, $edition , $yearofprint, $author1, $author2, $author3, $author4, $publisher,$blurb,$listprice, $ourprice,$shiptime,$pages)
{
	...
	mysqli_query($link,$db_query);
        ...
        $booklink = createbooklink($link, $title);	
...
function buildindex($link, $isbn10, $isbn13, $title, $edition , $yearofprint, $author1, $author2, $author3, $author4, $publisher,$blurb,$listprice, $ourprice,$vipkeys,$shiptime)
{
	...
	if ($shiptime == "Out Of Stock")
		$weight = 7;
	$value = $isbn10 . " " . $isbn13 . " " . $title . " " . $edition . " " . $author1 . " " . $author2 . " " . $author3 . " " . $author3 . " " . $author4 . " " . $publisher . " " . $vipkeys;
...
$keylines = file("sep-24-2010-books.data");
...
foreach($keylines as $line)
{
		...
		if ($param == "###")
		{
			...
			{
				$t = strstr($listprice, "INR");
				$nlp = "";
				if ($t != null)
				{
					$currency = "";
					$nlp = strstr($listprice, ":");
					$nlp = substr($nlp, 1);
					$listprice = trim($nlp);
				...
				{
					$t = strstr($listprice, "dollar");
					$nlp = "";
					if ($t != null)
					{
						$currency = "dollar";
						$nlp = substr($listprice,6);
						$listprice = trim($nlp);
					}
					else
					{
						echo "<br>no currency assuming rupee<br>";
					}
				}
			}
...
function createbooklink($linkdb, $str)
{
        $ostr = $str;
